...
void drv8838::flip(const int index) {
...
void StartModule::wait() {
	while(!digitalRead(start) && working==0) {
...
void StartModule::set(int mode) {
	...
	if(mode==1) working = 1;
	if(mode==2) {
		bool startState = !digitalRead(start);
		if(startState) working = 0;
		else working = 1;
	}
}
